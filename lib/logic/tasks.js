  ...
  try {
    if (req.method === 'GET') {
      ...
      tasks = tasks.filter(t => {
        ...
        if (!t.doneAt) return true; // Keep legacy done tasks for now or they will disappear
        return (now - t.doneAt) < 24 * 60 * 60 * 1000;
      });
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'create') {
        const tasksHash = await redis.hgetall(`tasks:${coupleId}`);
        const existingTasks = tasksHash ? Object.values(tasksHash).map(t => typeof t === 'string' ? JSON.parse(t) : t) : [];
        const isDuplicate = existingTasks.some(t => t.title === title && t.creatorId === user.id && (Date.now() - t.createdAt) < 5000);
        
        if (isDuplicate) return res.status(200).json({ status: 'duplicate' });
